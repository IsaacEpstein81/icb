...
if (videoSlot) {
  const previewUrl = videoSlot.dataset.previewUrl?.trim();
  const driveUrl = videoSlot.dataset.driveUrl?.trim();
  const title = videoSlot.dataset.title?.trim() || "Proposal walkthrough video";

  if (previewUrl) {
    videoSlot.classList.remove("media-card__frame--placeholder");
    videoSlot.innerHTML = `<iframe src="${previewUrl}" title="${title}" allow="autoplay" allowfullscreen loading="lazy"></iframe>`;
  }
  ...
  if (driveLink) {
    if (driveUrl) {
      ...
      driveLink.rel = "noreferrer";
      driveLink.removeAttribute("aria-disabled");
      driveLink.classList.remove("text-link--disabled");
    } else {
      driveLink.addEventListener("click", (event) => {
        event.preventDefault();
      });
    }
  }
